"""
Phase 9 Step 3 – Component Ablation (10 epochs × 4 configs)
Adapted to actual MuKG project structure (src.py.* imports).
Outputs per-epoch summary with neg_std, step_std, MRR, Hits@10.
"""
...
# ==================== Single Config ====================
def run_ablation(label, use_gpu, sorter, packer,
                 train_triples, all_triples_set, eval_triples,
                 n_entities, n_relations, cost_table,
                 epochs=10, batch_size=5000, neg_num=150):
    print(f"\n{'='*60}")
    ...
    out_dir = f'output/results/phase9_step3/{label}'
    os.makedirs(out_dir, exist_ok=True)
    summary_path = os.path.join(out_dir, 'summary.csv')

    with open(summary_path, 'w', newline='') as sf:
        writer = csv.writer(sf)
        writer.writerow(['epoch', 'avg_loss', 'mrr', 'hits10',
                         'epoch_time_s', 'neg_time_mean_ms', 'neg_time_std_ms',
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(provider.iterate(train_triples)):
                ...
                if step % 30 == 0:
                    print(f"  [{label}] E{epoch} S{step:3d} loss={loss.item():.4f} "
                          f"neg={neg_time:.1f}ms step={step_time:.1f}ms")

            epoch_time = time.time() - t_start
            avg_loss = float(np.mean(epoch_losses))
            neg_mean = float(np.mean(neg_times))
            neg_std = float(np.std(neg_times))
            ...
            writer.writerow([epoch, f'{avg_loss:.6f}', f'{mrr:.4f}', f'{hits10:.4f}',
                             f'{epoch_time:.1f}',
                             f'{neg_mean:.1f}', f'{neg_std:.1f}',
                             f'{step_mean:.1f}', f'{step_std:.1f}'])
            print(f"  [{label}] Epoch {epoch}: loss={avg_loss:.4f} MRR={mrr:.4f} "
                  f"Hits@10={hits10:.4f} neg_std={neg_std:.1f}ms step_std={step_std:.1f}ms "
...
def main():
    ...
    for label, use_gpu, sorter, packer in configs:
        run_ablation(label, use_gpu, sorter, packer,
    ...
    print(f"{'='*60}")
    summary_file = 'output/results/phase9_step3/ablation_summary.csv'
    with open(summary_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['config', 'final_loss', 'mrr', 'hits10',
                         'epoch_time_s', 'neg_std_ms', 'step_std_ms'])
        for label, _, _, _ in configs:
            sp = f'output/results/phase9_step3/{label}/summary.csv'
            if os.path.exists(sp):
                with open(sp) as sf2:
                    rows = list(csv.DictReader(sf2))
                last = rows[-1]
                writer.writerow([
                    label, last['avg_loss'], last['mrr'], last['hits10'],
                    last['epoch_time_s'], last['neg_time_std_ms'], last['step_time_std_ms']
                ])
                print(f"  {label:10s} | loss={last['avg_loss']} | MRR={last['mrr']} | "
                      f"Hits@10={last['hits10']} | time={last['epoch_time_s']}s | "
                      f"neg_std={last['neg_time_std_ms']}ms | step_std={last['step_time_std_ms']}ms")
