...
**Architecture Definition Status — HOLD**: The project currently contains three
inconsistent architecture descriptions:

- `paper_story_freeze.md`: four layers
  (FeatureExtractor → CostModel → Scheduler → BatchProvider) with GPU sampling
  embedded into the path.
- `paper/draft/method.md` §3.5: five layers
  (FeatureExtractor → CostModel → Scheduler → BatchProvider → GPU Sampler).
- `docs/runtime_framework_spec.md` §4: five modules
  (CostModel → Scheduler → RuntimePolicy → GPUExecution → BatchProvider).

The source tree implements FeatureExtractor, CostModel functions, Scheduler,
BatchProvider, and GPUNegativeSampler, but contains no independent
`RuntimePolicy`/`GPUExecution` class. Part 3 must freeze the paper architecture
before C2.1 can be activated.

### Claim Inventory

| # | Claim | Experiment | Figure/Table | Script | CSV Data |
|---|-------|-----------|-------------|--------|----------|
| **C2.1 — HOLD** | Candidate architecture claim: freeze one layer definition that distinguishes implemented modules from design-only RuntimePolicy/GPUExecution concepts | Phase 7 Step 4–5; Phase 8 Step 0 | Architecture diagram blocked on definition freeze | Design/source inspection | `docs/gpu_runtime_architecture.md`; `docs/phase8_architecture_freeze.md`; `docs/runtime_framework_spec.md`; `paper/draft/method.md` §3.5 |
...
| **C2.4 — ACTIVE** | The cost model is implemented as a deterministic offline table-building function and runtime cost access is a precomputed array lookup; no “zero” or “negligible” end-to-end overhead is asserted | Phase 5.5 / Phase 6 | — | `scripts/fit_cost_model.py`; `src/py/load/cost_model.py` | `output/results/cost_table.npy`; `output/results/cost_model_summary.md` |
...
**Coverage Notes**:
- C2.1 is on hold because the story freeze, Method draft, runtime specification,
  and implementation do not use one consistent layer definition. The
  architecture diagram must wait for Part 3.
- C2.4 is an implementation statement about offline construction and O(1)
  lookup. It must not absorb Scheduler traversal/packing overhead or be rewritten
  as “negligible” without a direct measurement.
...
| # | Claim | Experiment | Figure/Table | Script | CSV Data |
|---|-------|-----------|-------------|--------|----------|
| **C3.1 — HOLD** | Candidate measured-cost claim: candidate_size explains a reported R²=0.9008 on 455 sampled observations; target provenance, sampling unit, and leakage must be reconstructed before use | Phase 5.5 / Phase 6 | — (Fig.2 uses the retracted `hub_count` variable) | `scripts/fit_cost_model.py` | `docs/cost_model.md`; `output/results/cost_model_summary.md`; `output/results/cost_model_data.md` |
| **C3.2 — ACTIVE** | In the Phase 6 CBP run, predicted batch weight and measured negative-sampling time have a reported Pearson r=0.7124; no causal interpretation or baseline-path association is implied | Phase 6 runtime attribution | —; attribution report | `src/py/experiments/runtime_attribution.py` | `output/results/runtime_attribution/runtime_attribution.csv`; `output/results/runtime_attribution/attribution_report.md`; `output/results/runtime_attribution/attribution_interpretation.md` |
...
- Fig.2 cannot support C3.1: its generator reads `hub_count` and
  `neg_sampling_time` from `profiling_summary.csv`, whereas C3.1 concerns
  `candidate_size` and a reported R²=0.9008. The plotted variable belongs to the
  retracted C3.5 lineage.
...
- C3.6 corrects the paper draft's hypothetical float64 estimate to the actual
  float32 artifact.
- No current C3 Claim asserts measured cross-model generalization or “0.01ms
  prediction accuracy”. `output/results/phase9_step3/model_sensitivity.csv` does
  not exist, and the planned RotatE/ConvE experiment was not executed. C3.3 is
  limited to the implementation fact that the table construction uses static KG
  features; it is not empirical cross-model validation.

---

## C4: Cost-aware Batch Packing (CBP) — Pluggable Sort+Pack Scheduling

**Contribution Summary** (audit-safe revision of `paper_story_freeze.md` §Q3, §Q4):
CBP provides a pluggable Sort+Pack scheduling interface. A single Phase 6 run at
batch_size=1000 reports a 78% reduction in neg-time std, while Phase 9 Step 4.5
at batch_size=5000 reports only 8.4%. The former remains HOLD pending repeated
validation; CBP is positioned as an intermediate result motivating GPU migration.

**⚠️ Conditional Strength Note**: The apparent batch_size=1000 effect is a
single-run observation, not yet a statistically established strong result. At
batch_size=5000, the measured effect is marginal. No standalone highlight claim
is active.
...
| # | Claim | Experiment | Figure/Table | Script | CSV Data |
|---|-------|-----------|-------------|--------|----------|
| **C4.1 — HOLD** | A Phase 6 single-run experiment at batch_size=1000 reports neg-time std reduction from 15.5ms to 3.4ms (78%); this is empirical, not proof of theoretical correctness | Phase 6 runtime attribution | — (Fig.3 does not plot neg-time std) | `src/py/experiments/runtime_attribution.py` | `output/results/runtime_attribution/runtime_attribution.csv`; `output/results/runtime_attribution/attribution_report.md` |
...
| **C4.3 — ACTIVE** | Phase 9 Step 4.5 reports BL 29.5ms versus CBP 27.0ms neg-time std at batch_size=5000 (8.4% reduction); proposed noise explanations remain hypotheses | Phase 9 Step 4.5 | Table 4 (Fig.6 reads Phase 9 Step 3, not Step 4.5) | `src/py/experiments/phase9_step4_5_cpu_variance.py` | `output/results/phase9_step4_5/variance_summary.csv`; `output/results/phase9_step4_5/neg_sampling_variance.csv` |
...
| **C4.7 — HOLD** | Phase 6 Node 3.5 reports mean within-batch cost CV of 0.0552 for Baseline and 0.0124 for CBP at batch_size=5000 over two deterministic-layout epochs; this is within-batch heterogeneity, not inter-batch balance or runtime variance | Phase 6 Node 3.5 integration validation | — (Fig.3 plots inter-batch avg_cost, a different metric) | `src/py/experiments/validate_cbp_integration.py` | `output/results/integration_validation/validation_summary.json`; `output/results/integration_validation/batch_composition.csv`; `output/results/integration_validation/scheduler_trace.md` |

**Coverage Notes**:
- C4.1 (78% reduction): Single-run data; no independent-run uncertainty at
  batch_size=1000 is available.
- C4.1 and C4.7 are distinct: C4.1 measures per-batch runtime dispersion at
  batch_size=1000; C4.7 measures the average within-batch dispersion of predicted
  costs at batch_size=5000.
...
- Fig.3 currently plots inter-batch `avg_cost` distributions and reports
  Baseline CV=0.0008 versus CBP CV=0.0735. It does not visualize C4.1 or C4.7
  and cannot be used as evidence for cost-variance reduction without redesign.
...
| Fig.2 | `paper_assets/figures/fig2_cost_model_corr.pdf` | Retracted C3.5 lineage | None | ⚠️ Generated from `hub_count` and `neg_sampling_time`; does not plot C3.1 `candidate_size` or R²=0.9008 |
| Fig.3 | `paper_assets/figures/fig3_batch_cost_distribution.pdf` | C4 candidate | None pending lineage correction | ⚠️ Generated, but plots inter-batch `avg_cost` CV (Baseline 0.0008; CBP 0.0735), not C4.1 neg-time std, C4.7 within-batch cost CV, or C3.2 runtime correlation |
...
| Fig.6 | `paper_assets/figures/fig6_ablation_variance.pdf` | C1 + C4 | C1.3, C4.4 | ✅ Generated from Phase 9 Step 3; does not support Step 4.5 C4.3 |
...
- **Contradiction**: The story freeze, Method draft, runtime specification, and
  implementation use different four-/five-layer boundaries. C2.1 remains HOLD
  until Part 3 freezes one paper-facing definition and distinguishes implemented
  modules from design-only concepts.
...
- **Correction**: Fig.2 uses `hub_count` rather than C3.1's
  `candidate_size`, so its active Claim mapping is removed.
- **Gap**: Runtime correlation r=0.71 is moderate; gap between offline and online acknowledged.
- **Gap**: No cross-model sensitivity artifact exists at
  `output/results/phase9_step3/model_sensitivity.csv`; the planned RotatE/ConvE
  experiment was not executed, so no empirical cross-model generalization claim
  is inventoried.
...
- **Gap**: C4.7 is a two-epoch deterministic-layout observation of mean
  within-batch predicted-cost CV at batch_size=5000. It is not a replicated
  runtime-variance result and remains HOLD.
- **Correction**: Fig.3 plots inter-batch `avg_cost` CV, where the stored figure
  reports Baseline 0.0008 and CBP 0.0735. Its former mapping to C4.1, C4.7, and
  C3.2 is removed pending Part 5 redesign or relabeling.
- **Correction**: Fig.6 reads Phase 9 Step 3 summaries; its former mapping to
  Phase 9 Step 4.5 C4.3 is removed.
...
| ACTIVE candidates proceeding to audit | 16 |
| HOLD candidates requiring safer wording or stronger evidence | 10 |
...
*End of Evidence Audit Part 1. Version 1.2 freezes the candidate-claim registry
