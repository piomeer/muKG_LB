# Phase X X0.5 — Legacy Narrative Quarantine Design

**Date**: 2026-08-07
**Status**: Approved design; revised after written-spec review
**Scope**: Documentation provenance and claim-propagation control only

## 1. Purpose

Phase X has established canonical evidence that supersedes portions of the
historical paper narrative.  This design prevents those legacy documents from
being accidentally treated as current paper truth while Parts 4 and 5 decide
the status of CostModel and CBP claims.

The quarantine is an evidence-control layer.  It does not revise the paper,
alter the frozen Part 1 inventory, create a new empirical result, or repair
the underlying implementation.

## 2. Authority Model

The implementation will define this precedence order:

1. `docs/phase_x_x0_research_freeze.md` for research questions, scope,
   contribution hierarchy, estimands, and reporting constraints.
2. Evidence-audit reports and their machine-readable artifacts for audited
   claim grades and numerical evidence: Part 2 (C1), Part 3 (C2), then future
   Parts 4 and 5.
3. `docs/evidence_audit_part1_claim_inventory.md` for frozen historical claim
   identifiers and lineage only; its ACTIVE/HOLD/RETRACTED labels are not
   A/B/C/D grades.
4. Historical narrative, draft, and architectural documents for context only.
   They are non-authoritative until a Part 7 propagation pass updates them.

No document can silently override a source that is higher in this order.

## 3. Deliverables

### 3.1 Central quarantine register

Create `docs/phase_x_x0_5_legacy_narrative_quarantine.md` with nine sections:

1. safe writing sources;
2. purpose, execution boundary, and authority order;
3. document-status register;
4. data-source status register;
5. high-risk claim propagation matrix plus a Claim-ID reverse index;
6. writing prohibitions and release-status tracker;
7. Part 7 handoff;
8. release procedure; and
9. reproduction.

The document-status register will classify each inspected document as one of:

- **CANONICAL**: current source of truth for its declared subject;
- **FROZEN-INVENTORY**: preserves historical claim identifiers and lineage,
  but does not itself establish validity;
- **PENDING-AUDIT**: may contain future candidates, but cannot supply a paper
  conclusion before its designated audit; or
- **LEGACY-NON-AUTHORITATIVE**: contains historical wording or values that must
  not be copied into the manuscript without a canonical-source check.

The Safe Writing Sources section will list the current canonical documents and
artifacts explicitly:

- `docs/phase_x_x0_research_freeze.md`;
- `docs/evidence_audit_part2_c1_gpu_runtime.md`;
- `docs/evidence_audit_part3_c2_framework.md`;
- `docs/unified_runtime_architecture_freeze.md`;
- `output/results/evidence_audit_part2/`;
- `output/results/evidence_audit_part3/`; and
- `output/results/c1_r1_combined_rerun/`.

The last path is the checked-in name of the C1-R1 v1.1 evidence directory.
`output/results/c1_r1_v1.1/` does not exist and will not be documented as a
source.

### 3.2 Legacy-document page headers

Add a short, identical-format quarantine header immediately after the title of
these legacy documents:

- `paper/draft/method.md`;
- `docs/paper_outline.md`;
- `docs/paper_story_freeze.md`;
- `docs/runtime_framework_spec.md`;
- `docs/phase8_architecture_freeze.md`.

The header will link to X0 and the central register, state that the file is
historical/non-authoritative during Phase X, and instruct readers to use the
audits before reusing any numerical, architecture, or contribution claim.  It
will not change substantive body text, headings, figures, code, or artifacts.
It will be an HTML comment containing the exact machine-detectable marker:

```html
<!-- LEGACY-NON-AUTHORITATIVE: Phase X X0.5 -->
```

`docs/unified_runtime_architecture_freeze.md` is deliberately excluded from
this list: Part 3 declares it canonical for the implementation architecture,
so the status register will identify it as a supporting canonical source rather
than quarantine it.

### 3.3 Claim propagation matrix

Every row will contain: a stable quarantine ID, topic/legacy expression,
affected Part 1 Claim IDs, affected documents/data, current disposition,
canonical source, paper-use rule, audit owner, current release state, and
release condition.  The initial matrix covers:

| ID | Part 1 Claim IDs | Topic | Current disposition |
|---|---|---|---|
| Q-01 | C1.1 | 198x component speedup | Excluded; Phase 8 comparator is synthetic |
| Q-02 | C1.4 | 8.5x Phase 8 step speedup | Excluded; same comparator problem |
| Q-03 | C1.2 | 5.7x Phase 9 epoch speedup | Superseded by C1.2-R1 / E1 |
| Q-04 | C1.3 | 142x negative-time dispersion | Superseded by C1.3-R1 / E2 |
| Q-05 | C3.1 | R2=0.9008 / 90% explained | Pending Part 4 provenance and out-of-sample audit |
| Q-06 | C2.4, C2.6 | CostModel/scheduler zero or negligible overhead | Only deterministic lookup and protocol-specific measurements are established |
| Q-07 | C4.1, C4.3, C4.4, C4.7 | CBP 78% or 8.4% effects | Pending Part 5; fixture equivalence blocks contribution wording |
| Q-08 | C2.1 | Four/five-layer and RuntimePolicy/GPUExecution descriptions | Superseded by C2.1-R1 and the two-stage, five-role boundary |
| Q-09 | C1.5, C1.8, C4.5, C4.6 | CPU/GPU or scheduler quality equivalence/non-inferiority | Prohibited; current evaluations do not support it |
| Q-10 | C1.6, C1.9, C2.5 | Sampler-only VRAM, bottleneck shift, DDP-ready, general/SOTA claims | Unsupported or out of scope under X0 |
| Q-11 | C2.3 | Transparent migration, automatic injection, or drop-in/pluggable backend wording | Prohibited; backend selection is explicit and configuration-driven |

The exact wording will preserve mathematical symbols with plain-text fallbacks
where necessary (for example, `R^2`).

The register will also contain a reverse index from each affected Part 1 Claim
ID to its quarantine row.  Part 1 itself remains byte-for-byte unchanged:
editing the frozen inventory to add a column or `SUPERSEDED` label would destroy
the baseline against which Parts 2–7 record replacements.

### 3.4 Data-source status register

The register will distinguish current evidence from audit-only legacy data:

- **CURRENT-EVIDENCE**: `output/results/c1_r1_combined_rerun/` and the
  machine-readable Part 2/3 audit directories;
- **LEGACY-DATA / AUDIT-ONLY**:
  `output/results/unified_runtime/`, `output/results/phase9_step2/`, and
  `output/results/phase9_step3/` for superseded C1 performance/dispersion
  claims; and
- **PENDING-AUDIT-DATA**:
  `output/results/runtime_attribution/`,
  `output/results/phase9_step4_5/`, and relevant integration-validation
  artifacts for C3/C4.

New C1 manuscript analysis must read from C1-R1 or the Part 2 derived outputs.
Legacy data may still be read for historical auditing, C2.2 lineage work, or a
specifically registered claim, but the consuming script/document must name the
legacy source and its protocol limitation explicitly.  X0.5 does not rename,
delete, or modify any result artifact.

### 3.5 Deterministic quarantine checker

Create `scripts/check_x0_5_quarantine.py` with a standard-library-only CLI:

```text
python3 scripts/check_x0_5_quarantine.py
python3 scripts/check_x0_5_quarantine.py --repo-root .
python3 scripts/check_x0_5_quarantine.py --repo-root . \
  --output output/results/evidence_audit_x0_5/quarantine_checks.json
```

The checker will verify the five exact legacy headers, Q-01 through Q-11,
Claim-ID mappings, release fields, safe-source paths, and data-directory
classifications.  Output is deterministic JSON without timestamps.  A failed
check returns a non-zero exit code.  The checker is a standalone X0.5 gate; it
will be invoked by Part 7 rather than modifying the already-completed Part 3
audit script.

## 4. Data Flow and Interfaces

The quarantine register consumes only existing documents and audit artifacts.
It produces documentation-level routing instructions:

```
legacy narrative occurrence
  -> quarantine matrix row
  -> Part 1 Claim ID and replacement lineage
  -> canonical evidence source
  -> permitted wording or prohibition
  -> named Part 4/5/7 owner and release condition
```

There is no training API, runtime API, GPU process, or experiment input/output
in this work.  `PROGRESS.md` and `mukg-memory.json` will record completion and
the next mandatory gate (X1.5) without changing the scientific claims.

## 5. Failure Handling

- If a legacy document contains a high-risk expression not covered by a matrix
  row, classify it as `UNMAPPED` and add a row before declaring X0.5 complete.
- If two canonical sources disagree, record the conflict and defer to the
  higher-precedence source; do not reconcile the science in X0.5.
- If a future audit changes a claim, Part 7 updates the quarantine row and
  lifts or retains the header; X0.5 does not silently rewrite the legacy body.
- If a document is absent, record it as `NOT-PRESENT` rather than inventing a
  lineage record.
- No row is released merely because a replacement result exists.  Release
  requires the named audit grade/decision and a Part 7 propagation action.

## 6. Validation and Acceptance

Implementation is accepted only when:

1. every required legacy file exists and has exactly one HTML-comment
   quarantine marker;
2. each of Q-01 through Q-11 has affected Claim IDs, one disposition, one
   canonical source, one audit owner/state, and one explicit release condition;
3. every affected Part 1 Claim ID resolves through the reverse index without
   modifying Part 1;
4. the register names the full authority order, Safe Writing Sources, data
   source classifications, and the Part 1 status/grade distinction;
5. no substantive legacy body text, code, result artifact, figure, or paper
   claim is rewritten;
6. every referenced local path exists;
7. `rg` checks confirm that the designated high-risk terms are either mapped or
   present only in canonical evidence/register explanations;
8. unit tests prove missing/duplicated headers, missing Q rows, nonexistent
   source paths, and incomplete release fields fail closed;
9. two checker runs produce byte-identical JSON;
10. `git diff --check` passes; and
11. the worktree has no unrelated changes before commit.

## 7. Explicit Non-Goals

- No GPU or CPU experiment, benchmark, profiling run, or code change.
- No rewrite of Method, Introduction, Results, figures, or the story freeze.
- No Part 4/Part 5 scientific conclusion, literature review, or contribution
  promotion.
- No alteration of Part 1's frozen rows or Claim IDs.
- No push to GitHub; publishing remains a separate user-authorized action.

## 8. Handoff

After X0.5, the mandatory research sequence remains X1.5 systematic mapping
and novelty audit, Part 4 C3 audit, Part 5 C4 audit, contribution triage, and
then Part 7 manuscript propagation.  The quarantine register is the required
input to Part 7 so that only released claims enter the regenerated manuscript.
